...
export const useGetCondominiosQueryOptions = (empresaId:number, {
  search,
  page,
  limit,
  exclude,
  ...
  return queryOptions({
    queryKey: ['condominios', empresaId, { search, page, limit, exclude }, queryKeys],
    queryFn: () => getCondominios(empresaId, { search, page, limit, exclude })
  })
}
...
export default function ListarCondominios({
  ...
  limitView: number
  exclude: string
  onSelectCondominio: ((condominio: Condominio) => void) | undefined
}) {
  const { user } = useAuth();
  ...
  const [showcard, setShowCard] = useState(!!onSelectCondominio);
  ...
  const handleClickCreateCondominio = () => {
    navigate(ROUTE.CONDOMINIOS_CRIAR)
  }
  // UI Logic
  ...
  return (
    <div className="container mx-auto space-y-4 p-4 font-[Poppins-regular]">
      ...
      <div className={(isBigScreen ? "grid gap-4 grid-cols-3" : isPortrait ? "grid gap-4 grid-cols-3" : isTablet ? "grid gap-4 grid-cols-2" : isMobile ? "grid gap-4 grid-cols-1" : "grid gap-4 grid-cols-1")}>
        ...
        {hasSearchResults && (
          <p className="text-center text-muted-foreground">Nenhum condomínio encontrado</p>
        )}
              ...
              (
                <>
                  {condominios?.map((condominio) => (
                    <Card key={condominio.id} className="">
